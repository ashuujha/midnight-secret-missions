/**
 * Deploy the Cat Bluff contract to a Midnight network.
 *
...
if (WALLET.created) {
  console.log(
    "A new deployment wallet was saved in .midnight-state.json (gitignored). Back it up privately before funding.",
  );
}
...
const zkConfigPath = path.resolve(__dirname, "..", "managed", "cat-bluff");
...
const witness =
  (field: keyof DeployState) => (context: { privateState: DeployState }) =>
    [context.privateState, context.privateState[field]] as const;
const gameWitnesses = {
  identitySecret: witness("secret"),
  handCounts: witness("counts"),
  handSalt: witness("salt"),
  nextHandSalt: witness("nextSalt"),
  playedCat: witness("cat"),
  playedSalt: witness("cardSalt"),
  drawnCats: witness("draws"),
};

const compiledContract = CompiledContract.make("cat-bluff", Game.Contract).pipe(
...
async function createProviders(walletCtx: WalletContext) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: "cat-bluff-state",
      accountId,
...
async function main() {
  ...
  console.log(`║  Deploy Cat Bluff to ${network}`);
  console.log(
    "╚══════════════════════════════════════════════════════════════╝\n",
  );
  ...
  if (dustState.dust.balance(new Date()) === 0n) {
    ...
    try {
      ...
      console.log(
        "  DUST is generated by registered NIGHT UTXOs and pays transaction fees.",
      );
      console.log("  Common causes:");
      console.log(
        "    • The node is not producing blocks — check: docker compose ps",
      );
      console.log(
        "    • The wallet holds no NIGHT — check: npm run check-balance",
      );
      if (network !== "undeployed") {
        console.log(
          `    • The ${network} faucet has not funded this address yet`,
        );
      }
      console.log("");
      await walletCtx.wallet.stop();
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      // The constructor uses no witnesses; this initial state only satisfies
      // the contract type expected by Midnight.js during deployment.
      deployed = await deployContract(providers, {
        ...
        initialPrivateState: {
          ...
          nextSalt: new Uint8Array(32),
          cat: 0n,
          cardSalt: new Uint8Array(32),
  ...
  console.log(`  Cat Bluff is live on ${network}.\n`);
}
