...
If **Deploy with Lace** fails while a `/check` request is pending or fails, inspect the proof-server URL in Lace's Midnight settings. Deployment has no game-circuit proof to send through the dApp's provider; Lace uses its own configured prover to balance the transaction. In Lace 2.4, open **Settings → Midnight Settings → Proof Server**, select **Remote** if offered, then **Save** and reconnect the wallet. Lace's Local option points to `http://localhost:6300` and requires a running local proof server. The Lace settings screen offers preset Local/Remote choices rather than an arbitrary URL field. The dApp can separately use the shared [`Render prover`](https://midnight-counter-prover.onrender.com/ready) through `VITE_PROOF_SERVER_URL`; this does not update Lace. If `/check` still fails, note its HTTP status and host; a healthy `/ready` response alone does not prove that a wallet transaction can be balanced.
